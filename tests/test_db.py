...
def test_migration_creates_indexes_declared_on_new_columns(tmp_path, monkeypatch):
    path = tmp_path / "leasure.db"
    con = sqlite3.connect(path)
    con.execute("CREATE TABLE tracks (id INTEGER PRIMARY KEY, title VARCHAR(500), artist VARCHAR(500))")
    con.commit()
    ...
    assert "isrc" in {c["name"] for c in inspect(engine).get_columns("tracks")}
    indexes = {ix["name"] for ix in inspect(engine).get_indexes("tracks")}
    assert "ix_tracks_isrc" in indexes

    # Second run finds nothing pending: no new index, no new backup
    with engine.begin() as conn:
        db_mod._migrate_missing_columns(conn)
    assert {ix["name"] for ix in inspect(engine).get_indexes("tracks")} == indexes
    assert len(list(tmp_path.glob("leasure.db.*.bak"))) == 1
